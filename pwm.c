...
PWM_RETURN_E pwm_setDutyCycle(TIMER_TypeDef *timer, uint8_t ch, uint32_t dutyCycle1000)
{
  ...
  if (dutyCycle1000 > 1000) {
      return PWM_INVALID_PARAM;
  }
  uint32_t top = TIMER_TopGet(timer);
  uint32_t compare = (top+1) * dutyCycle1000 / 1000;
...
uint32_t pwm_getDutyCycle(TIMER_TypeDef *timer, uint8_t ch) {
    uint32_t top = TIMER_TopGet(timer);
    uint32_t dutyCycle1000 = timer->CC[ch].OC * 1000 / (top + 1);
    return dutyCycle1000;
}
...
PWM_RETURN_E pwm_setPwmFreq(TIMER_TypeDef *timer, uint32_t pwmFreq)
{
  ...
  for (int i = 0; i < 3; i++) {
      uint32_t dutyCycle1000 = timer->CC[i].OC * 1000 / (currentTop + 1);
...
uint32_t pwm_getPwmFreq(TIMER_TypeDef *timer) {
    ...
    uint32_t clockFreq = CMU_ClockFreqGet(clock);
    uint32_t top = TIMER_TopGet(timer);
    uint32_t pwmFreq = clockFreq / (top + 1);
    return pwmFreq;
}
...
{
  if ((pwmInit->numOfChsUsed > 3) || (pwmInit->numOfChsUsed < 1)
      || (pwmInit->pwmFreq < 650)
      || (pwmInit->ch[0].dutyCycle1000 > 1000)
      || (pwmInit->ch[1].dutyCycle1000 > 1000)
      || (pwmInit->ch[2].dutyCycle1000 > 1000)) {
      return PWM_INVALID_PARAM;
  }
  ...
  for (int i = 0; i < pwmInit->numOfChsUsed; i++) {
      GPIO_PinModeSet(pwmInit->ch[i].port, pwmInit->ch[i].pin, gpioModePushPull, 0);
      GPIO->TIMERROUTE_SET[timerID].ROUTEEN = (1UL << i);
  }

  switch (pwmInit->numOfChsUsed) {
    case 1:
     GPIO->TIMERROUTE_SET[timerID].CC0ROUTE = ((pwmInit->ch[0].pin << 16) | pwmInit->ch[0].port);
     break;
    case 2:
     GPIO->TIMERROUTE_SET[timerID].CC0ROUTE = ((pwmInit->ch[0].pin << 16) | pwmInit->ch[0].port);
     GPIO->TIMERROUTE_SET[timerID].CC1ROUTE = ((pwmInit->ch[1].pin << 16) | pwmInit->ch[1].port);
     break;
    case 3:
     GPIO->TIMERROUTE_SET[timerID].CC0ROUTE = ((pwmInit->ch[0].pin << 16) | pwmInit->ch[0].port);
     GPIO->TIMERROUTE_SET[timerID].CC1ROUTE = ((pwmInit->ch[1].pin << 16) | pwmInit->ch[1].port);
     GPIO->TIMERROUTE_SET[timerID].CC2ROUTE = ((pwmInit->ch[2].pin << 16) | pwmInit->ch[2].port);
     break;
  }
